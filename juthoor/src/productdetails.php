...
<a href="new.html"><i class='fa fa-shopping-cart fa-2x  white-color fa-2xl' ></i></a>
</div>
...
try{
    include 'config.php'; 
    $ID=$_GET['ID'];    
     $results = $conn->query("SELECT * FROM Plants where Plant_ID=$ID");
  ...
  while($row = $results->fetch_assoc()) {
      ...
      echo ' <div class="ac">
        ...
        <button class="bnn btnnn ">Add to Cart</button>
      </div>';
